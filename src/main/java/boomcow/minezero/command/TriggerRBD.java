                        ...
                        .executes(context -> {
                            ...
                            if (anchorPlayerUUID == null) {
                                context.getSource().sendFailure(Component.literal(
                                        "No anchor player (Subaru player) is set. Cannot trigger Return By Death."));
                            ...
                            if (anchorPlayer == null) {
                                ...
                                context.getSource().sendFailure(Component.literal("The anchor player (UUID: "
                                        ...
                                        + ") is not currently online. Cannot trigger Return By Death with current implementation."));
                                LOGGER.warn("Attempted to trigger RBD for anchor {}, but player is not online.",
                                        anchorPlayerUUID);
                                return 0;
                            }

                            context.getSource().sendSuccess(
                                    () -> Component.literal("Manually triggering Return By Death for anchor player: "
                                            + anchorPlayer.getName().getString() + ". World will reset to checkpoint."),
                                    true);
                            LOGGER.info(
                                    "Manually triggering Return By Death for anchor player: {} (UUID: {}) by command sender: {}",
                            ...
                            level.getServer().execute(() -> {
                                ...
                                level.getServer().getPlayerList().getPlayers().forEach(p -> {
                                    p.displayClientMessage(Component.literal(
                                            "Return By Death has been manually triggered! Resetting to the last checkpoint."),
                                            false); 
                                });
